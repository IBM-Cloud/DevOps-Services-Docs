...
Build jobs compile your project in preparation for deployment. Artifacts that are generated by build jobs can be sent to a build archive directory, though by default, they is placed in the project's root directory. Jobs that take input from build jobs should reference build artifacts in the same structure that they were created. For example, if a build job archives build artifacts to an `output` directory, a deploy script would need to reference that rather than the project root directory to deploy the compiled project.
...
Deploy jobs can deploy entirely new apps, or update existing ones. This is true even if you first deployed an app using other methods, like the run bar in the Web IDE or the Cloud Foundry command-line tool. To update an existing app, use its name in the deploy job. 
...
You can deploy a previous builds of an app by selecting it in the Stage History menu, clicking **SEND TO**, and choosing a target.
